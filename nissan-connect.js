...
class LeafConnect {
  // NE = Eurupa
  constructor(username, password, region = 'NE') {
  ...
  async login() {
    ...
    console.log('gdcUserId: ' + res.gdcUserId);
    this.leaf = res.leaf;
  ...
  async getBatteryStatus() {
    ...
    while (updateInfo === null) {
      ...
      [updateInfo] = await Promise.all([
        ...
        LeafConnect.timeout(5000) //wait 5 seconds before continuing
      ]);
    }
    return updateInfo;
  }
  ...
  async acOn() {
    ...
    while (updateInfo === null) {
      console.log('retrying ac requestResult');
      [updateInfo] = await Promise.all([
